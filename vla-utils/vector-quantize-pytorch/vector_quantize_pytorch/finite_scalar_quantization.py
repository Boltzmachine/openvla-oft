...
class FSQ(Module):
    ...
    def bound(self, z, eps = 1e-3, return_unquantized=False):
        ...
        if return_unquantized:
            return round_ste(bounded_z) / half_width, bounded_z / half_width
    ...
    def quantize(self, z, return_unquantized=False):
        ...
        bounded_z = bound_fn(z, return_unquantized=return_unquantized)
        if return_unquantized:
            bounded_z, unquantized_z = bounded_z
        ...
        if not self.training or noise_dropout == 0.:
            if return_unquantized:
                return bounded_z, unquantized_z
            return bounded_z
    ...
    def forward(self, z, return_distances=False):
        """
        einstein notation
        b - batch
        ...
        with quantization_context():
            ...
            codes = self.quantize(z, return_unquantized=return_distances)
            if return_distances:
                codes, unquantized_codes = codes
                unquantized_codes = rearrange(unquantized_codes, 'b n c d -> b n (c d)')
                dists = -torch.cdist(unquantized_codes, self.implicit_codebook.to(unquantized_codes.dtype), p=2)
