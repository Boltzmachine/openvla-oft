...
def cdist(x, y, eps=1e-7):
    x2 = reduce(x ** 2, 'b n d -> b n', 'sum')
    y2 = reduce(y ** 2, 'b n d -> b n', 'sum')
    xy = einsum('b i d, b j d -> b i j', x, y) * -2
    return ((rearrange(x2, 'b i -> b i 1') + rearrange(y2, 'b j -> b 1 j') + xy).clamp(min = 0) + eps).sqrt()
...
class EuclideanCodebook(Module):
    ...
    def forward(
        ...
        if exists(codebook_transform_fn):
            ...
            dist = -torch.cdist(flatten.float(), embed.float(), p=1).to(dtype)
...
class VectorQuantize(Module):
    ...
    def forward(
        ...
        x = self._codebook.transform_input(x).to(x.dtype)
        ...
        if not return_loss_breakdown:
            if return_distances:
                return quantize, embed_ind, loss, distances
