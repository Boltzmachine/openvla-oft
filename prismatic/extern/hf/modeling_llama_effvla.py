    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if layer_idx == STARTING_IDX:
                assert cache_position.size(0) > 1
                attentions = layer_outputs[1]
                
                image_hidden_states = hidden_states[:, :256]
                mask_base = torch.zeros(inputs_embeds.size(0), 256, device=inputs_embeds.device, dtype=torch.bool)
                scores = attentions.mean(1)[:, 256:, :256].sum(-2)
                index_key_task = torch.topk(scores, K_key + K_task, dim=-1).indices
                index_key = index_key_task[:, :K_key]
                index_task = index_key_task[:, K_key:]
                mask_key = mask_base.clone().scatter_(1, index_key, True)
                mask_task = mask_base.clone().scatter_(1, index_task, True)
                
                mask_rem = ~ (mask_key | mask_task)
                V_key_dense = image_hidden_states * mask_key.unsqueeze(-1)
                V_rem_dense = image_hidden_states * mask_rem.unsqueeze(-1)
                valid_mask = (mask_key.unsqueeze(1).float() * mask_rem.unsqueeze(2).float()).bool()
                cos = nn.functional.cosine_similarity(V_rem_dense.unsqueeze(2), V_key_dense.unsqueeze(1), dim=-1)
                cos = cos.masked_fill(~valid_mask, float("-inf"))
                diversity = 1 - cos.max(-1).values
                diversity = diversity.masked_fill(~mask_rem, float("-inf"))
                index_div = torch.topk(diversity, K_div, dim=-1).indices
                mask_div = mask_base.clone().scatter_(1, index_div, True)
                mask_pruned = mask_key | mask_task | mask_div
                assert mask_pruned[0].sum() == K_final, f"mask_pruned: {mask_pruned[0].sum()} != {K_final}"
                assert causal_mask.dim() == 4, "Causal mask should be 4D for Gemma2 models."
                mask_pruned_all = torch.cat(
                    [mask_pruned, torch.ones(inputs_embeds.size(0), inputs_embeds.size(1) - 256, device=inputs_embeds.device, dtype=torch.bool)], dim=1
                )
                mask_pruned_all_padded = torch.cat([mask_pruned_all, torch.zeros(inputs_embeds.size(0), causal_mask.size(-1) - mask_pruned_all.size(-1), device=inputs_embeds.device, dtype=torch.bool)], dim=1)
                causal_mask = causal_mask.masked_fill(~mask_pruned_all[:, None, :, None], causal_mask.min()).masked_fill(~mask_pruned_all_padded[:, None, None, :], causal_mask.min())

            assert hasattr(self, "skip_layers")
...
class LlamaForCausalLMEffVLA(LlamaPreTrainedModel, GenerationMixin):
