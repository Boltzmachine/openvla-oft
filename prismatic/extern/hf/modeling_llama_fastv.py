...
class LlamaModelFastV(LlamaPreTrainedModel):
    """
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if output_hidden_states:
                ...
                if USE_FASTV:
                    # past_key_value = (past_key_values.key_cache[idx], past_key_values.value_cache[idx]) if past_key_values is not None else None
                    ...
                    if idx==AGG_LAYER:
                        if past_seen_tokens == 0:
                            ...
                            # keep index
                            keep_indexs = torch.cat( (torch.arange(SYS_LENGTH,device=device), top_attention_rank_index, torch.arange(SYS_LENGTH+IMAGE_TOKEN_LENGTH,past_key_values.key_cache[0].shape[2],device=device)))
                            # sort index
                            keep_indexs = keep_indexs.sort().values
                            # use top_attention_rank_index to compute
                            # new_past_key_value.key_cache = past_key_value[0][:,:,keep_indexs,:]
                            # new_past_key_value.value_cache = past_key_value[1][:,:,keep_indexs,:]
                        else:
                            ...

                    elif idx > AGG_LAYER:
                        # past_key_value = new_past_key_value)
                        if past_seen_tokens == 0 and hidden_states.size(1) != keep_indexs.size(0):
                            hidden_states = hidden_states[:, keep_indexs]
                            position_embeddings = None if position_embeddings is None else (position_embeddings[0][:, keep_indexs], position_embeddings[1][:, keep_indexs])
                            causal_mask = causal_mask[:, :, keep_indexs][...,keep_indexs]
                            position_ids = keep_indexs
        ...
        next_cache = (keep_indexs, next_cache)
...
class LlamaForCausalLMFastV(LlamaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModelFastV(config)
        self.vocab_size = config.vocab_size
